...
pub fn get_expenses_chart(monthly_table: &MonthlyTable) -> Chart {
    ...
    for (pos, row) in monthly_table.rows.iter().enumerate() {
        ...
        let previous_expenses = if pos > 0 { monthly_table.rows[pos - 1].expenses } else { Decimal::zero() };
        series_monthly_expenses.push([date, (row.expenses - previous_expenses).to_f64().unwrap()]);

        let last_sma_expenses = if pos > sma_size { monthly_table.rows[pos - sma_size].expenses } else { Decimal::zero() };
        series_monthly_expenses_sma.push([date, (row.expenses - last_sma_expenses).to_f64().unwrap() / (sma_size as f64)]);
    }
